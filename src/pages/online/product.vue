    ...
    <main class='scroll-content'>
      <section class="info-1">
        ...
        <p class="danger-color fs-16 flex just-between align-center">
          ...
          <template v-else>
            <span>零售价：￥{{info.productAttrStock&&info.productAttrStock.price}}</span>
          </template>
      ...
      <section class="info-2">
        <yd-cell-group>
          ...
          <template v-else>
            <yd-cell-item>
              <span slot="left" class="primary-color">普通商家兑换：{{info.productAttrStock&&formatPrice(info.productAttrStock.standardTotalAmount*10)}}
                <span class="iconfont self-quan"></span>
              </span>
            </yd-cell-item>
            <yd-cell-item>
              <span slot="left" class="royal-color">VIP商家兑换：{{info.productAttrStock&&formatPrice(info.productAttrStock.honourTotalAmount*10)}}
                <span class="iconfont self-quan"></span>
              </span>
            </yd-cell-item>
          </template>
      ...
      if (this.buyType == 0) {
        ...
        mui.ajax({
          ...
          success(res) {
            ...
            vm.$router.push({
              name: "SettleBalance",
              query: { orderType: vm.orderType, buynow: true, tips }
            });
          },
          error(e) {
            vm.$dialog.loading.close();
